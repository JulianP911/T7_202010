...
public class SeparteChainingHashST <K extends Comparable<K>, V >
{
	/**
	...
	public SeparteChainingHashST(int m) 
	{
		this.m = m;
	...
	private void resize(int chains)
	{
		...
		SeparteChainingHashST<K, V> temp = new SeparteChainingHashST<K, V>(capacidad);
		for (int i = 0; i < m; i++) 
		{
			for (K key : st[i].keys()) 
	...
	// return values in symbol table as an Iterable
	public Iterable<V> Vals() 
	{
		LinkedQueue<V> queue = new LinkedQueue<V>();
		for (int i = 0; i < m; i++)
		{
			for (V value : st[i].vals())
				queue.enqueue(value);
		}
		return queue;
	} 
